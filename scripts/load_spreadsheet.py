...
inserts rows into projects, interconnection, and environmental tables.
...
def build_column_map(headers):
    ...
    m["purchase_price_k"] = find_col(headers, "Purchase Price*", "Purchase Price ($k)")
...
def g(row, col_map, field, default=None):
...
def insert_project(cur, row, col_map):
    name = clean(g(row, col_map, "project_name"))
    gis_id_raw = clean(g(row, col_map, "gis_id"))
    ...
    lat, lon = parse_latlon(g(row, col_map, "latlon"))

    values = [
        gis_id, name,
        clean(g(row, col_map, "market")),
        clean(g(row, col_map, "tech")) or "BESS",
        to_num(g(row, col_map, "project_size_mw")),
        to_num(g(row, col_map, "project_duration_mwh")),
        clean(g(row, col_map, "land_agent")),
        clean(g(row, col_map, "paces_link")),
        clean(g(row, col_map, "address")),
        lat, lon,
        clean(g(row, col_map, "state")),
        clean(g(row, col_map, "county")),
        clean(g(row, col_map, "ahj")),
        clean(g(row, col_map, "parcel_id")),
        to_num(g(row, col_map, "total_acreage")),
        to_num(g(row, col_map, "buildable_acreage")),
        clean(g(row, col_map, "site_control_status")),
        to_num(g(row, col_map, "risk_score_site_control")),
        to_num(g(row, col_map, "risk_score_permitting")),
        to_num(g(row, col_map, "risk_score_design")),
        to_num(g(row, col_map, "risk_score_overall")),
        clean(g(row, col_map, "priority_tier")),
        clean(g(row, col_map, "update_site_control")),
        clean(g(row, col_map, "update_permitting")),
        clean(g(row, col_map, "update_design")),
        clean(g(row, col_map, "action_items")),
        clean(g(row, col_map, "big_rock")),
        clean(g(row, col_map, "key_risks")),
        clean(g(row, col_map, "next_steps")),
        clean(g(row, col_map, "mitigation_plan")),
        clean(g(row, col_map, "glint_fit_check")),
        clean(g(row, col_map, "site_conditions")),
        clean(g(row, col_map, "site_visit_notes")),
        clean(g(row, col_map, "permitting_consultant_notes")),
        to_date(g(row, col_map, "loi_sent")),
        to_date(g(row, col_map, "epf_submitted")),
        to_date(g(row, col_map, "epf_approved")),
        clean(g(row, col_map, "epf_on_hold")),
        to_num(g(row, col_map, "option_length_years")),
        clean(g(row, col_map, "option_payments_k")),
        to_num(g(row, col_map, "lease_rate_k")),
        to_num(g(row, col_map, "annual_escalator_pct")),
        to_num(g(row, col_map, "purchase_price_k")),
        to_num(g(row, col_map, "project_bid_value_k")),
        clean(g(row, col_map, "zoning_designation")),
        clean(g(row, col_map, "by_right")),
        clean(g(row, col_map, "sup_cup")),
        clean(g(row, col_map, "energy_community")),
        clean(g(row, col_map, "highly_impacted_community_wa")),
        clean(g(row, col_map, "vulnerable_population_wa")),
        clean(g(row, col_map, "low_income_community")),
        clean(g(row, col_map, "dac_justice40")),
        clean(g(row, col_map, "opportunity_zone")),
        clean(g(row, col_map, "nmtc_eligible")),
        clean(g(row, col_map, "incentive_zone_md")),
        clean(g(row, col_map, "arcgis_link")),
        clean(g(row, col_map, "task_board_link")),
        clean(g(row, col_map, "dev_plan_link")),
    ]

    cols = ", ".join(PROJECT_COLUMNS)
    placeholders = ", ".join(["%s"] * len(PROJECT_COLUMNS))
    sql = f"""
...
def insert_interconnection(cur, gis_id, row, col_map):
    utility = clean(g(row, col_map, "ix_utility"))
    if not utility and not clean(g(row, col_map, "ix_priority_feeder")):
        return
    ...
    """, (
        gis_id,
        utility,
        clean(g(row, col_map, "ix_priority_feeder")),
        to_num(g(row, col_map, "ix_poi_hcm")),
        to_num(g(row, col_map, "ix_mw_queued_feeder")),
        to_num(g(row, col_map, "ix_mw_queued_sub")),
        clean(g(row, col_map, "ix_two_subs")),
        clean(g(row, col_map, "ix_feeder_id")),
        clean(g(row, col_map, "ix_feeder_section_id")),
        clean(g(row, col_map, "ix_sub_loading")),
        clean(g(row, col_map, "ix_nwa")),
        to_num(g(row, col_map, "ix_poi_voltage")),
        clean(g(row, col_map, "ix_poi_distance")),
        clean(g(row, col_map, "ix_sub_name")),
        to_num(g(row, col_map, "ix_poi_distance_sub")),
        clean(g(row, col_map, "ix_notes")),
        to_num(g(row, col_map, "ix_single_max")),
        to_num(g(row, col_map, "ix_double_max")),
        to_num(g(row, col_map, "ix_net_grid_cap")),
        clean(g(row, col_map, "ix_circuit_mva")),
        to_num(g(row, col_map, "ix_hcm_thermal")),
        clean(g(row, col_map, "ix_feeder_elevation")),
        to_num(g(row, col_map, "ix_upgrade_score")),
        clean(g(row, col_map, "ix_num_easements")),
    ))


def insert_environmental(cur, gis_id, row, col_map):
    slope = clean(g(row, col_map, "env_slope"))
    wetlands = clean(g(row, col_map, "env_wetlands"))
    ...
    """, (
        ...
        slope,
        wetlands,
        clean(g(row, col_map, "env_flood")),
        clean(g(row, col_map, "env_vegetation")),
        clean(g(row, col_map, "env_soil")),
        clean(g(row, col_map, "env_tribal")),
        clean(g(row, col_map, "env_species")),
        clean(g(row, col_map, "env_site_fit")),
    ))
